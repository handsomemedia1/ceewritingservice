...
export default function AdminOrders() {
  ...
  const getStatusPill = (status: string) => {
    switch (status) {
      ...
      case 'delivered': return <span style={{ background: '#f1f5f9', color: 'var(--text-muted)', padding: '4px 10px', borderRadius: '50px', fontSize: '12px', fontWeight: 600 }}>Delivered to Client</span>;
      default: return <span style={{ background: '#f1f5f9', color: 'var(--text-muted)', padding: '4px 10px', borderRadius: '50px', fontSize: '12px', fontWeight: 600 }}>{status}</span>;
    }
  };
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border)', overflow: 'hidden' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', gap: '16px' }}>
          <div style={{ flex: 1, position: 'relative' }}>
            ...
            <input type="text" placeholder="Search tasks by client name..." style={{ width: '100%', padding: '12px 12px 12px 42px', borderRadius: '8px', border: '1px solid var(--border)', outline: 'none' }} />
          </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: '64px 24px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'var(--bg-main)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)', marginBottom: '16px' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            ...
            <tbody>
              {orders.map(order => (
                <tr key={order.id} style={{ borderBottom: '1px solid var(--border)', transition: 'background 0.2s', cursor: 'pointer' }} onClick={() => window.location.href=`/dashboard/orders/${order.id}`}>
