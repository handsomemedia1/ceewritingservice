      ...
      <div className="container mx-auto px-6 max-w-5xl">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="max-w-2xl">
            <span className="text-text-primary/70 font-bold tracking-wider uppercase mb-3 block text-sm">Advanced Research</span>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {hubs.map((hub) => (
            <div key={hub.title} className="bg-transparent border-2 border-border/10 hover:border-border/30 transition-all p-8 flex flex-col h-full rounded-none">
              <div className="w-16 h-16 rounded-full bg-bg-main/5 flex items-center justify-center text-3xl mb-6">
                {hub.icon}
              </div>
              <h3 className="text-2xl font-bold font-serif text-text-primary mb-3">{hub.title}</h3>
              <p className="text-muted leading-relaxed mb-6 flex-grow">{hub.desc}</p>
              
              <div className="flex flex-wrap gap-2 mb-8">
                {hub.tools.map(tool => (
                  <span key={tool} className="px-3 py-1 bg-bg-card border border-border/10 rounded-full text-xs font-bold text-text-primary/70">
                    {tool}
                  </span>
                ))}
              </div>

              <Link href={hub.href} className="mt-auto inline-flex items-center text-sm font-bold text-text-primary hover:text-text-primary/70 transition-colors uppercase tracking-wider">
